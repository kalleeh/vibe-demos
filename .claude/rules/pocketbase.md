...
- **SDK version: pin exactly, never `@latest`** — but pin a *recent* version. Query Context7 (`/pocketbase/js-sdk` for SDK tag, `/pocketbase/pocketbase` for core) for current latest, then pin that.
  - **State (verified 2026-09-06, re-verify via Context7):** server binary `/opt/pocketbase/pocketbase` is `0.40.2`. `backends/config.json` registers 10 backends; every frontend pins the JS SDK at `0.28.1` (clinic-admin verifies it with a sha384 hash before importing); `ai` and `changwon-homes` have no frontend SDK.2`; `ai` (hook-only proxy) and `changwon-homes` (data-upload chute only — its `upload.html` pins `0.26.2`; the live page has no SDK) ship no frontend SDK. SDK `0.25.x`/`0.26.x` talk to a `0.40.2` server fine. Core has reached `0.35.x`.
