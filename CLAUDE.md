...
- `contraption-lab/` ("Contraption Lab") — a modern *Incredible Machine*: drag parts to build Rube Goldberg contraptions that satisfy each level's goal, then Run the physics. Matter.js (vendored single file, no build step), split into focused ES modules under `js/`; 10 official levels in `js/levels/official.js` (data — all verified solvable by a headless Matter sweep), versioned level JSON in `js/level.js` (the editor/PocketBase contract), 4 swappable CSS-token themes in `js/theme.js` (Blueprint default). localStorage progress; pure-logic tests run via `?test`. **Phases 1–2 shipped** — Phase 2 added optional PocketBase accounts (port 8100, `pb/pb_migrations/001_init_progress.js`; the `progress` collection IS the synced progress + per-level leaderboard, public-read/owner-write; `js/cloud.js` defers the SDK import + is local-first so signed-out/offline plays fully; `js/auth-ui.js` is the account panel). Phase 3 (community level editor) still planned. Spec: `docs/superpowers/specs/2026-06-20-contraption-lab-design.md`.
