...
- `clinic-admin/` — Korean 한방병원 administration workspace, PoC on demo data only. PIN-locked, encrypted-at-rest ES-module app (`js/core`, `js/security`, `js/tabs`): 상병코드 정비, 자보 심사결과 대조, 연말정산·비급여·보존 점검표, 면허/인증 trackers, ⌘K palette, redaction-gated AI coding assist, E2E-encrypted intake board. Bundled code tables are 예시; real KOICD/심평원 masters can be uploaded. Maintenance contract: run `node clinic-admin/tools/check-sw-shell.mjs`, `node clinic-admin/tools/i18n-extract.mjs` (0 missing keys) and `node clinic-admin/tools/e2e.mjs` (+ `--mobile`) after any change. KO | EN toggle (`?lang=en`); Korean is the source of truth, English must stay complete.
