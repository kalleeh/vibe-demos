...
- `emberfall/` ("Emberfall") — gothic isometric action RPG on Three.js r186 WebGPU (WebGL2 fallback), TSL shaders; town / moor / procedural three-floor dungeon with a three-phase boss, loot, skill tree, gamepad, localStorage saves under `emberfall.*`. **Exception to the no-build rule:** this folder is the checked-in `dist/` of a separate Vite + TypeScript source repo (~59 MB incl. CC0 textures/audio, relative `base: "./"`). Never hand-edit the hashed bundles; rebuild there (`npm run build`) and rsync `dist/` here.
